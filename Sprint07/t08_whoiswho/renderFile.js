...
function getIndex(insert = false){
	try{
		...
		const ret = data && insert ? data.replace('#TEXT#', insert) : data.replace('#TEXT#', '');
		return ret;
	}
	catch (err){
		console.error(err);
...
function renderTable(arr, filter = false){
	...
	let check = filter && Object.keys(filter).length;
	if (check !== 0){
		arr = arr.filter((item) => {
			let flag = true;
			for (let key in item){
				let checkSecond = (filter[key] === item[key] || filter[key] === 'all-items');
...
{
	let result = `<select name="${title}">${title}<option value="all-items" ${!filter 
		|| filter === 'all-items' ? 'selected' : ''}><b>${title} (all)</b></option>`;
